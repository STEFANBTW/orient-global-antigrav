...
const MenuScreen: React.FC = () => {
    ...
    const continentalMenu = menuBlock.continental || [];
    const grillMenu = menuBlock.grill || [];
    const drinksMenu = menuBlock.drinks || [];
    const dessertMenu = menuBlock.desserts || [];
            ...
            <section id="continental" className="py-12 bg-slate-50 dark:bg-slate-900 border-y border-slate-200 dark:border-slate-800">
                ...
                <div className="overflow-x-auto no-scrollbar pb-8 pl-4 md:pl-[calc((100vw-80rem)/2)]">
                    <div className="flex space-x-6 w-max px-4">
                        {continentalMenu.map((item: any, idx: number) => (
                            <ContinentalCard
                                key={idx}
                                image={item.image}
                                tag={item.tag}
                ...
                <div className="max-w-4xl mx-auto px-6 mt-16 space-y-8">
                    ...
                    <div className="flex justify-between items-center border-b border-gray-200 dark:border-white/10 pb-4">
                        <div className="w-full">
                            {grillMenu.map((item: any, idx: number) => (
                                <div key={idx} className="flex justify-between items-center border-b border-gray-200 dark:border-white/10 pb-4 mb-4 last:border-0 last:mb-0 last:pb-0">
                                    <div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {drinksMenu.map((drink: any, idx: number) => (
                                    <div key={idx} className={`relative group rounded-xl overflow-hidden aspect-[3/4] cursor-pointer ${idx % 2 !== 0 ? 'mt-16' : ''}`}>
                                        <img src={drink.image} alt={drink.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                        <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                                            <span className="text-2xl font-bold text-white tracking-widest uppercase border-b-2 border-primary pb-1">{drink.title}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <section id="dessert" className="py-12 bg-slate-50 dark:bg-slate-950 text-slate-900">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid gap-8">
                        {dessertMenu.map((item: any, idx: number) => (
                            <DessertItem
                                key={idx}
                                image={item.image}
                                title={item.title}
