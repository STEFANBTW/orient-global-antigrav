...
const Landing: React.FC<LandingProps> = ({ onChangeView }) => {
  ...
  const heroBlock = cmsData?.blocks?.find((b: any) => b.block_type === 'hero')?.content_payload || {
    title: "LEVEL UP",
    subtitle: "YOUR REALITY",
    highlight: "NEURAL LINK ACTIVE"
  };
      ...
      <section className="relative h-screen w-full flex items-center justify-center overflow-hidden bg-[#050505]">
        ...
        <div className="relative z-10 text-center max-w-6xl mx-auto px-4 w-full">
          ...
          >
            ...
            <span className="font-mono text-[10px] text-primary tracking-[0.3em] uppercase">{heroBlock.highlight || "Neural Link Active"}</span>
          </motion.div>

          <motion.h1
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="font-mono text-6xl md:text-[9rem] font-black text-transparent bg-clip-text bg-gradient-to-b from-white via-gray-300 to-gray-600 mb-2 leading-[0.85] tracking-tighter drop-shadow-[0_0_30px_rgba(255,255,255,0.1)] whitespace-pre-wrap"
          >
            {heroBlock.title}
          </motion.h1>
          ...
          >
            <h2 className="font-mono text-4xl md:text-7xl font-bold text-primary tracking-tight relative z-10 whitespace-pre-wrap">
              {heroBlock.subtitle}
            </h2>
            <div className="absolute -inset-4 bg-primary/20 blur-2xl z-0"></div>
            {/* Glitch text effect behind */}
            <h2 className="absolute top-0 left-[2px] font-mono text-4xl md:text-7xl font-bold text-cyan-500 tracking-tight z-0 opacity-50 mix-blend-screen whitespace-pre-wrap">{heroBlock.subtitle}</h2>
            <h2 className="absolute top-0 -left-[2px] font-mono text-4xl md:text-7xl font-bold text-red-500 tracking-tight z-0 opacity-50 mix-blend-screen whitespace-pre-wrap">{heroBlock.subtitle}</h2>
          </motion.div>
