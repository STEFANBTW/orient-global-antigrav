...
export const VRFrontier: React.FC<Props> = ({ onNavigate }) => {
    ...
    const vrBlock = cmsData?.blocks?.find((b: any) => b.block_type === 'vr')?.content_payload || {
        games: [
            {
                id: 'game-1',
                title: 'Lagos Saber',
                description: 'Slice through the beats in a futuristic world. Experience the adrenaline of music manifesting as physical objects.',
                image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBoCjiYS6IdAhEPA7bgRgiHJbFEuY5O34utIcdITPs5OGgnVyV0-TH4ro3s6dZnVCi6GuIcIu8Nm_-AhE3GfZe2pRPzXFJWLefjJ4_54qvYt8atlI4hrpHppfapltnxi7LxT1Nl-wuML43S_4NcAuS7l4kzV2m6aE7G8PrElOoYn8BRlpsXSbakncw7aH1AraXSd8oRoh8LMUJfQKwgAJxMM7rSXpjbjYlKy1-T8nHVamOCLZSqdX0ti4DvfRM9TycDtbDwnlhGvYRM',
                intensity: 'HIGH',
                comfort: 'STABLE',
                statsLabel2: 'Minimal Motion Sickness',
                tag: 'Rhythm Action'
            },
            {
                id: 'game-2',
                title: 'Sahara Heat',
                description: 'Survive the apocalypse under the scorching sun. Every bullet counts. Immersive haptic feedback makes every shot feel real.',
                image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCo6SbAy_quBrJUcl6xaDeIEHSeW8Bz_YyxbqixFrL7BM0LP18qmGr19HnAO5iulAlwRCCvmQn-L2SHTk7yeLC0orayoyc_ol1lGNU4S59Ipmh-feBd29Wk9GsrEY4lpMbxe_UnCga8bvpw7C-ks_8yMYeXXpSX3AaQ2HkV8wT4KfckIiDrSGLIJ9UoCOT476xuHhwew0D7FxCpkJOM2uldah2K9qg2T9U09iAwljsoOJoGw_CM5HRa3hnd4svTLhzR4XwKd1XriHp1',
                intensity: 'EXTREME',
                comfort: 'MODERATE',
                statsLabel2: 'Locomotion Movement',
                tag: 'Survival Horror'
            },
            {
                id: 'game-3',
                title: 'SUPER HOT',
                description: 'Time moves only when you move. Blur the lines between strategy and mayhem in this stylized shooter.',
                image: 'https://www.transparenttextures.com/patterns/carbon-fibre.png', // Fallback for 3rd visual
                intensity: 'MEDIUM',
                comfort: 'HIGH',
                statsLabel2: 'Stationary scale',
                tag: 'Tactical Puzzle'
            }
        ]
    };
                ...
                <div className="max-w-[1920px] mx-auto px-4 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center w-full">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-[#0df2f2] tracking-widest text-[10px] font-bold uppercase">{vrBlock.games[0].tag}</span>
                        </div>
                        <h2 className="text-5xl md:text-7xl font-black text-white uppercase leading-tight tracking-tighter">
                            {vrBlock.games[0].title.split(' ')[0]} <br /><span className="text-transparent stroke-text" style={{ WebkitTextStroke: '2px #0df2f2' }}>{vrBlock.games[0].title.split(' ').slice(1).join(' ')}</span>
                        </h2>
                        <p className="text-gray-400 text-xs uppercase tracking-widest max-w-md leading-relaxed">
                            {vrBlock.games[0].description}
                        </p>
                        ...
                        <div className="grid grid-cols-2 gap-4 max-w-md">
                            ...
                            <div className="bg-[#111] border border-white/10 p-4 hover:border-[#0df2f2]/50 transition-colors" style={{ clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }}>
                                ...
                                <div className="text-lg font-black text-white uppercase tracking-wider mb-1">{vrBlock.games[0].comfort}</div>
                                <span className="text-[8px] text-gray-500 uppercase tracking-widest">{vrBlock.games[0].statsLabel2}</span>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        <img alt={vrBlock.games[0].title} className="w-full h-full object-cover transform transition-transform duration-700 group-hover:scale-105 filter contrast-125" src={vrBlock.games[0].image} />
                ...
                <div className="max-w-[1920px] mx-auto px-4 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center w-full">
                    ...
                    >
                        <div className="flex items-center justify-end lg:justify-start gap-4">
                            <span className="text-[#0df2f2] tracking-widest text-[10px] font-bold uppercase">{vrBlock.games[1].tag}</span>
                            <div className="h-[1px] w-12 bg-[#0df2f2]"></div>
                        ...
                        <h2 className="text-5xl md:text-7xl font-black text-white uppercase leading-tight tracking-tighter">
                            {vrBlock.games[1].title.split(' ')[0]} <br /><span className="text-transparent stroke-text" style={{ WebkitTextStroke: '2px #ef4444' }}>{vrBlock.games[1].title.split(' ').slice(1).join(' ')}</span>
                        </h2>
                        <p className="text-gray-400 text-xs uppercase tracking-widest ml-auto lg:ml-0 max-w-md leading-relaxed">
                            {vrBlock.games[1].description}
                        </p>
                        ...
                        <div className="grid grid-cols-2 gap-4 max-w-md ml-auto lg:ml-0">
                            ...
                            <div className="bg-[#111] border border-white/10 p-4 text-left hover:border-red-500/50 transition-colors" style={{ clipPath: 'polygon(10px 0, 100% 0, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0 100%, 0 10px)' }}>
                                ...
                                <div className="text-lg font-black text-white uppercase tracking-wider mb-1">{vrBlock.games[1].comfort}</div>
                                <span className="text-[8px] text-gray-500 uppercase tracking-widest">{vrBlock.games[1].statsLabel2}</span>
                            </div>
                        </div>
                ...
                <div className="max-w-[1920px] mx-auto px-4 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center w-full">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-white tracking-widest text-[10px] font-bold uppercase">{vrBlock.games[2].tag}</span>
                        </div>
                        <h2 className="text-5xl md:text-7xl font-black text-white uppercase leading-tight tracking-tighter">
                            {vrBlock.games[2].title.split(' ')[0]} <br /><span className="text-[#0df2f2] italic">{vrBlock.games[2].title.split(' ').slice(1).join(' ')}</span>
                        </h2>
                        <p className="text-gray-400 text-xs uppercase tracking-widest max-w-md border-l-2 border-[#0df2f2] pl-4 leading-relaxed">
                            {vrBlock.games[2].description}
                        </p>
