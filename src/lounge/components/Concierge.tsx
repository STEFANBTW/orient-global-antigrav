...
const Concierge: React.FC = () => {
  ...
  const conciergeBlock = cmsData?.blocks?.find((b: any) => b.block_type === 'concierge')?.content_payload || {
    intro: {
      subtitle: "Midnight Concierge",
      welcome: "Welcome back,",
      name: "Alexander."
    },
    card: {
      points: "12,450",
      memberId: "8821 9004 1120",
      status: "Noir Elite"
    },
    events: {
    ...
    history: {
      ...
      items: [
        { title: "Old Fashioned w/ Smoked Maple", desc: "Ordered 3 times this month", icon: "liquor" }
      ]
    }
  };
  ...
  return (
    <div className="bg-[#130f09] font-manrope text-white min-h-screen overflow-x-hidden p-8">
      <div className="max-w-6xl mx-auto pt-12">
        <div className="mb-12">
          <h2 className="text-primary/60 text-xs uppercase tracking-[0.3em] mb-4">{conciergeBlock.intro.subtitle}</h2>
          <h1 className="text-4xl md:text-6xl font-light tracking-wide text-white">{conciergeBlock.intro.welcome} <br /><span className="font-medium text-transparent bg-clip-text bg-gradient-to-r from-white to-white/60">{conciergeBlock.intro.name}</span></h1>
        </div>
        ...
        <div className="max-w-md w-full aspect-[1.586/1] bg-black border border-white/10 rounded-2xl p-8 relative overflow-hidden group mb-16">
          ...
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div>
              ...
              <div className="text-4xl font-bold text-primary mt-1">{conciergeBlock.card.points}</div>
            </div>
            ...
            <div className="flex justify-between items-end">
              <div>
                ...
                <p className="font-mono text-white/80 tracking-widest">{conciergeBlock.card.memberId}</p>
              </div>
              <span className="text-xs font-bold uppercase tracking-widest">{conciergeBlock.card.status}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-20">
          <div className="flex justify-between items-end mb-8">
            <div>
              <h3 className="text-primary text-xs font-bold uppercase tracking-widest mb-2">{conciergeBlock.events.subtitle}</h3>
              <h2 className="text-3xl font-light">{conciergeBlock.events.title}</h2>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {conciergeBlock.events.items.map((card: any, i: number) => (
        ...
        {/* History */}
        <div className="pb-24">
          <h2 className="text-3xl font-light mb-8">{conciergeBlock.history.title}</h2>
          <div className="space-y-4">
            {conciergeBlock.history.items.map((item: any, i: number) => (
              <div key={i} className="bg-neutral-900/50 p-6 rounded-xl border border-white/5 flex items-center justify-between">
