...
export default function FashionHub({ navigateTo, onProductClick }: { navigateTo: (view: string) => void, onProductClick?: (product: any) => void }) {
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
        ...
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-6 py-4 md:px-10">
          <div className="flex items-center gap-12">
            ...
            <nav className="hidden items-center gap-8 lg:flex">
              <a className="text-sm font-semibold hover:text-[var(--color-accent-light)] transition-colors" href="#">Shop</a>
              <a className="text-sm font-semibold hover:text-[var(--color-accent-light)] transition-colors" href="#">Editorial</a>
              <a className="text-sm font-semibold hover:text-[var(--color-accent-light)] transition-colors text-[var(--color-accent-light)]" href="#">Collections</a>
              <a className="text-sm font-semibold hover:text-[var(--color-accent-light)] transition-colors" href="#">Sale</a>
            </nav>
          </div>
          <div className="flex items-center gap-4">
            <div className="hidden md:flex relative group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[var(--color-accent-light)] transition-colors" size={20} />
      ...
      <section className="relative h-[90vh] w-full overflow-hidden bg-slate-900">
        ...
        <div className="relative z-20 flex h-full flex-col items-center justify-center px-6 text-center">
          <span className="mb-4 inline-block rounded-full bg-[var(--color-accent-light)]/20 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-[var(--color-accent-light)] backdrop-blur-md">Autumn/Winter 2024</span>
          <h1 className="max-w-4xl text-5xl font-black leading-tight text-white md:text-8xl">REDEFINING THE <span className="text-[var(--color-accent-light)] italic">ORANGE</span> ERA</h1>
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <button className="rounded-full bg-[var(--color-accent-light)] px-10 py-4 text-sm font-bold text-white transition-all hover:bg-orange-600 hover:scale-105">Shop the Drop</button>
      ...
      <footer className="bg-slate-900 text-white pt-24 pb-12">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-24 border-b border-white/10 pb-24">
          <div className="col-span-1 lg:col-span-1">
            <div className="flex items-center gap-2 text-[var(--color-accent-light)] mb-8">
              <Grid size={32} />
