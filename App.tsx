...
import { WaterApp, WaterNav, WaterPage, WaterProvider } from './src/water/WaterWrapper';
...
import { useBakeryCms } from './src/lib/bakeryCmsData';
import { ScrollContext } from './src/ScrollContext';
import { DynamicUtilityDock } from './src/components/ui/DynamicUtilityDock';
...
const Navbar: React.FC<{
...
}> = ({ theme, toggleTheme, setCurrentView, scrolled, isSubpage, navHidden, isAppNavHovered = false, onHoverChange, heroOutOfView = false }) => {
  ...
  // 3-State Logic for Cinematic Flow
  const isOpaque = heroOutOfView;
  const isBlurred = !heroOutOfView && (activeTab || isAppNavHovered || !navHidden);
  const isTransparentState = !isOpaque && !isBlurred;

  const navBackground = isOpaque
    ? 'var(--nav-opaque)'
    : (isBlurred ? 'var(--nav-blur)' : 'transparent');

  const navBlur = isBlurred ? 'backdrop-blur-xl' : '';

  const navBorder = isOpaque
    ? 'border-black/5 dark:border-white/10 shadow-soft'
    : (isBlurred ? 'border-white/10 dark:border-black/10' : 'border-transparent');
  ...
  return (
    <motion.div
      ...
      transition={{ duration: navHidden ? 0.3 : 0.6, ease: [0.22, 1, 0.36, 1] }}
    ...
    >
      ...
      <nav className={`relative transition-all duration-500 flex items-center 
          ${isMobile
          ? `rounded-[48px] h-14 sm:h-14 px-6 sm:px-8 ${isOpaque || isBlurred ? 'shadow-soft bg-white/5 dark:bg-zinc-900/5 backdrop-blur-sm border border-black/5 dark:border-white/10' : 'bg-transparent border-transparent'}`
          : `h-[56px] px-8 sm:px-16 border-b transition-all duration-300 ${navBlur} ${navBorder}`
        }`}
        style={{ backgroundColor: !isMobile ? navBackground : undefined }}
      >
        <div className={`${isMobile ? 'w-full flex items-center justify-between' : 'mx-auto w-full flex items-center justify-between'}`}>
          {/* Logo Section */}
          <div className={`flex items-center gap-3 sm:gap-4 cursor-pointer ${isMobile ? 'pl-2 pr-6 border-r border-white/10' : ''}`} onClick={() => { setCurrentView('home'); document.getElementById('main-scroll-container')?.scrollTo({ top: 0, behavior: 'smooth' }); }}>
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9px] sm:text-[11px] font-bold tracking-[0.3em] text-primary uppercase leading-none mt-0.5">Global</span>
            </div>
          </div>
            ...
            >
              {navLinks.map((link) => (
                <button
                  ...
                  className={`relative z-10 px-3 sm:px-4 h-full flex items-center justify-center transition-all duration-300 text-[clamp(0.9rem,1.3vw,1.1rem)] font-bold uppercase tracking-widest hover:text-primary ${textColor}`}
                >
          ...
          <div className={`flex items-center gap-1 sm:gap-3 ${isMobile ? 'pl-4 border-l border-white/10' : ''}`}>
            <button onClick={toggleTheme} className={`p-1.5 sm:p-2 rounded-full hover:bg-primary/10 transition-all duration-300 ${textColor}`}>
      ...
      <AnimatePresence>
        {(!isMobile && activeTab) && (
          <motion.div
            ...
            onMouseLeave={() => handleMouseLeave()}
            className={`absolute w-full border border-black/5 dark:border-white/10 backdrop-blur-xl shadow-[0_32px_64px_rgba(0,0,0,0.4)] z-[-1] overflow-hidden top-full left-0 rounded-none border-t-0 p-12 ${scrolled ? 'bg-white dark:bg-slate-900' : 'bg-white/5 dark:bg-black/5'}`}
          >
  ...
  return (
    <div ref={ref} className="relative h-screen flex flex-col items-center justify-center text-center px-6 overflow-hidden bg-transparent">
      ...
      <motion.div style={{ y: springYText, opacity }} className="content-container relative z-20 max-w-5xl">
        ...
        <ScrollItem index={1} className="font-heading font-black text-5xl sm:text-6xl md:text-8xl lg:text-[8.5rem] text-[var(--home-heading)] mb-8 tracking-tighter leading-[0.8] uppercase">
        ...
        <ScrollItem index={2}>
          <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-[var(--home-heading)] opacity-90 max-w-4xl mx-auto font-light tracking-wide mb-16 leading-relaxed drop-shadow-md dark:drop-shadow-lg">
...
const FinalCTA: React.FC = () => {
  return (
    <section className="relative flex flex-col justify-center bg-transparent border-t border-[var(--home-border)] items-center">
      <div className="content-container relative z-10 text-center">
        ...
        <h2 className="text-5xl md:text-7xl lg:text-8xl font-black text-[var(--home-heading)] mb-6 leading-[0.8] uppercase tracking-tighter">
          ORIENT<br />
          <span className="text-[var(--home-primary)]">Global</span>
        </h2>

        <Reveal>
          <p className="text-sm md:text-base text-[var(--home-text-muted)] max-w-xl mx-auto leading-relaxed font-medium mb-8">
        ...
        <Reveal>
          <div className="flex flex-wrap justify-center gap-4">
            <button className="px-6 py-3 bg-[var(--home-primary)] text-white font-black uppercase tracking-[0.2em] text-[10px] rounded-xl shadow-[0_10px_20px_rgba(242,158,13,0.3)] hover:scale-105 hover:shadow-[0_15px_30px_rgba(242,158,13,0.4)] transition-all duration-500">Visit Us Today</button>
          </div>
        </Reveal>
      </div>
    </section>
...
const BakeryShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="bakery" className="flex flex-col justify-center px-4 md:px-12 lg:max-w-[67vw] mx-auto bg-[var(--home-bg)] transition-colors duration-500 relative items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="md:w-1/2">
          <Reveal>
            ...
            <h2 className="text-4xl md:text-6xl font-black text-[var(--home-heading)] mb-6 font-heading leading-[0.85] uppercase tracking-tighter">
              GOLDEN<br /><span className="text-[var(--home-primary)]">Crust</span>
            </h2>
            <p className="text-[var(--home-text-muted)] text-base leading-relaxed mb-8 font-medium max-w-lg">Morning excellence starts here. From artisanal sourdoughs to delicate pastries, our bakery combines European techniques with local flavors like Coconut and Plantain.</p>
            <button onClick={() => { setCurrentView('bakery'); document.getElementById('main-scroll-container')?.scrollTo({ top: 0, behavior: 'smooth' }); }} className="group flex items-center gap-2 text-[var(--home-heading)] border-b-2 border-[var(--home-primary)] pb-1 hover:text-[var(--home-primary)] transition-all uppercase text-xs font-black tracking-[0.3em] font-heading">
              <span>View Menu</span>
    ...
    <div className="flex flex-col justify-center px-6 bg-transparent border-t border-black/5 dark:border-white/5 relative items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <Reveal animation='slide-from-right' className="order-1 lg:order-2">
          <span className="text-[var(--home-primary)] font-black tracking-[0.5em] uppercase text-[10px] mb-4 block">Artisanal Process</span>
          <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter mb-6 leading-tight">THE SCIENCE OF <br /><span className="text-[var(--home-primary)]">Fermentation</span></h2>
          <p className="text-base text-[var(--home-text-muted)] leading-relaxed mb-8 font-medium">Our master bakers utilize a 48-hour cold fermentation process, allowing complex flavors to develop naturally. We source our grains from sustainable farms, ensuring every loaf meets the Orient Global standard of purity.</p>
          <ul className="space-y-4">
            {['Natural Sourdough Starters', 'Stone-Ground Flour', 'No Artificial Additives'].map((item, i) => (
              <Reveal key={item} delay={`delay-${i * 300}`}>
                <li className="flex items-center gap-4 text-[var(--home-heading)] font-bold uppercase tracking-widest text-xs">
                  <span className="w-2 h-2 rounded-full bg-[var(--home-primary)] shadow-[0_0_15px_rgba(242,158,13,0.5)]" />
                  {item}
                </li>
              </Reveal>
            ))}
        ...
        <div className="md:w-1/2">
          <Reveal>
            ...
            <h2 className="text-4xl md:text-6xl font-black dark:text-white text-slate-900 mb-6 font-heading leading-[0.85] uppercase tracking-tighter">
              GLOBAL<br /><span className="text-[var(--home-primary)]">Standards</span>
            </h2>
      ...
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="flex flex-col">
          ...
          <Reveal animation='slide-from-right' delay="delay-200">
            <h2 className="text-4xl sm:text-5xl font-black dark:text-white text-slate-900 uppercase tracking-tighter mb-6 leading-tight">INSTITUTIONAL <br /><span className="text-[var(--home-primary)]">Quality</span></h2>
          </Reveal>
...
const RestaurantShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="restaurant" className="flex flex-col justify-center px-4 md:px-12 lg:max-w-[67vw] mx-auto bg-[var(--home-bg)] transition-colors duration-500 relative items-center h-screen cinematic-section">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="md:w-1/2">
          ...
          <Reveal delay="delay-200">
            <h2 className="text-4xl md:text-6xl font-black text-[var(--home-heading)] mb-6 font-heading leading-[0.85] uppercase tracking-tighter">
              CULINARY<br /><span className="text-[var(--home-primary)]">Artistry</span>
            </h2>
          </Reveal>
          <Reveal delay="delay-300">
            <p className="text-[var(--home-text-muted)] text-base leading-relaxed mb-8 font-medium max-w-lg">A symphony of flavors crafted by world-class chefs. From local delicacies like Pounded Yam & Egusi to international fusion, every dish is a masterpiece of taste and presentation.</p>
          </Reveal>
        ...
        <div className="md:w-1/2">
          <Reveal animation="slide-from-right">
            <ParallaxImage src="https://images.unsplash.com/photo-1514362545857-3bc16c4c7d1b?q=80&w=2070&auto=format&fit=crop" alt="Fine Dining Experience" className="rounded-3xl h-[50vh] border border-[var(--home-border)] shadow-elite" />
          </Reveal>
        </div>
      </div>
    </section>
...
const DiningDeepDive: React.FC = () => {
  return (
    <div className="flex flex-col justify-center px-6 bg-transparent border-t border-black/5 dark:border-white/5 relative items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <Reveal animation='rotate-in' className="order-2 lg:order-1 relative aspect-square rounded-3xl overflow-hidden border border-[var(--home-border)] shadow-elite">
        ...
        <Reveal animation='slide-from-right' className="order-1 lg:order-2">
          ...
          <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter mb-6 leading-tight">FUSION OF <br /><span className="text-primary">Heritage</span></h2>
          <p className="text-base text-[var(--home-text-muted)] leading-relaxed mb-8 font-medium">Our culinary team explores the intersection of traditional Plateau ingredients and modern gastronomic techniques. We believe in "Root-to-Table" dining, where every ingredient tells a story of the land.</p>
          <div className="space-y-4">
            {['Locally Sourced Produce', 'Artisanal Plating', 'Curated Wine Pairings'].map(item => (
              <div key={item} className="flex items-center gap-6 p-6 rounded-3xl bg-[var(--home-card-bg)] border border-[var(--home-card-border)] shadow-soft">
                <span className="material-icons text-[var(--home-primary)] text-2xl">restaurant_menu</span>
                <span className="text-[var(--home-heading)] font-bold uppercase tracking-widest text-xs">{item}</span>
              </div>
            ))}
          </div>
        </Reveal>
...
const WaterShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="water" className="flex flex-col justify-center px-4 md:px-12 lg:max-w-[67vw] mx-auto bg-[var(--home-bg)] transition-colors duration-500 relative items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="md:w-1/2">
          <Reveal>
            <span className="text-primary font-bold tracking-[0.3em] uppercase text-sm mb-4 block">Orient Water</span>
            <h2 className="text-5xl md:text-6xl font-black text-[var(--home-heading)] mb-6 font-heading leading-[0.85] uppercase tracking-tighter">
              PURE<br /><span className="text-primary">Hydration</span>
            </h2>
            <p className="text-[var(--home-text-muted)] text-base leading-relaxed mb-8 font-medium max-w-xl">The gold standard of purity. Our 7-step filtration process ensures every drop is as crisp and clean as nature intended. NAFDAC Certified quality for your peace of mind.</p>
            <button onClick={() => { setCurrentView('water'); document.getElementById('main-scroll-container')?.scrollTo({ top: 0, behavior: 'smooth' }); }} className="group flex items-center gap-4 text-[var(--home-heading)] border-b-2 border-primary pb-2 hover:text-primary transition-all uppercase text-xs font-black tracking-[0.3em] font-heading">
...
const WaterDeepDive: React.FC = () => {
  return (
    <div className="flex flex-col justify-center px-6 bg-transparent border-t border-[var(--home-border)] relative items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <Reveal animation='slide-from-left'>
          <div>
            <span className="text-primary font-black tracking-[0.3em] uppercase text-sm mb-4 block">Technical Purity</span>
            <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter mb-6 leading-tight">7-STEP <br /><span className="text-primary">Filtration</span></h2>
            <p className="text-base text-[var(--home-text-muted)] leading-relaxed mb-8 font-medium">Beyond standard purification, Orient Water undergoes a rigorous 7-step process including Reverse Osmosis, UV Sterilization, and Ozone Treatment. We test every batch in our on-site laboratory to ensure absolute safety.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {['Reverse Osmosis', 'UV Sterilization', 'Ozone Treatment', 'Mineral Balancing'].map(step => (
                <div key={step} className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--home-card-bg)] border border-[var(--home-card-border)] shadow-soft">
                  <span className="w-2 h-2 rounded-full bg-sky-500 shadow-[0_0_10px_rgba(14,165,233,0.5)]" />
                  <span className="text-[var(--home-heading)] text-[10px] font-black uppercase tracking-widest">{step}</span>
                </div>
              ))}
            </div>
          </div>
...
const CuratedExperiences: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="lounge" className="relative h-screen flex flex-col justify-center bg-[var(--home-bg)] transition-colors duration-500 items-center">
      <div className="lg:max-w-[67vw] mx-auto px-6">
        <div className="mb-16 text-center">
          <Reveal>
            ...
            <h3 className="text-5xl md:text-6xl font-serif text-[var(--home-heading)] italic font-light tracking-tight leading-none">Curated <br /><span className="not-italic font-display font-black opacity-40 uppercase tracking-tighter">Experiences</span></h3>
          </Reveal>
        </div>
        <div className="flex flex-col md:flex-row items-center gap-16">
          <div className="w-full md:w-1/2">
            <Reveal animation="slide-from-left">
              <ParallaxImage src="https://images.unsplash.com/photo-1470337458703-46ad1756a187?q=80&w=2069&auto=format&fit=crop" alt="Lounge Experience" className="rounded-3xl aspect-[4/5] border border-[var(--home-border)] shadow-elite" />
            </Reveal>
          </div>
          <div className="w-full md:w-1/2">
            <Reveal animation="slide-from-right">
              <div className="w-12 h-1 bg-primary mb-8 rounded-full"></div>
              <h4 className="text-4xl font-black text-[var(--home-heading)] mb-6 uppercase tracking-tighter leading-none">The <br /><span className="text-primary">Nightscape</span> Lounge</h4>
              <p className="text-[var(--home-text-muted)] text-base leading-relaxed mb-8 font-medium">Where mixology meets mystery. Our lounge offers a secluded environment perfect for high-stakes meetings or unwinding after a long week. Featuring Zobo-infused cocktails and a curated cigar selection.</p>
              <ul className="space-y-4 mb-8">
                {['Premium Bottle Service', 'Private Booths', 'Live Jazz Weekends'].map(li => (
                  <li key={li} className="flex items-center text-[var(--home-text-muted)] font-bold text-sm tracking-wide uppercase">
                    <span className="w-2 h-2 bg-primary rounded-full mr-4 shadow-[0_0_20px_rgba(242,158,13,0.5)]"></span>
                    {li}
                  </li>
                ))}
              </ul>
              <button onClick={() => { setCurrentView('lounge'); document.getElementById('main-scroll-container')?.scrollTo({ top: 0, behavior: 'smooth' }); }} className="group flex items-center gap-4 text-[var(--home-heading)] border-b-2 border-primary pb-2 hover:text-primary transition-all uppercase text-xs font-black tracking-[0.3em] font-heading">
...
const LoungeDeepDive: React.FC = () => {
  return (
    <div className="flex flex-col justify-center px-6 bg-transparent border-t border-[var(--home-border)] items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <Reveal animation='slide-from-left' className="order-2 lg:order-1 relative aspect-square rounded-3xl overflow-hidden border border-[var(--home-card-border)] shadow-3xl">
        ...
        <Reveal animation='slide-from-right' className="order-1 lg:order-2">
          ...
          <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter mb-6">THE NIGHTSCAPE <span className="opacity-40 italic">ETHOS.</span></h2>
          <p className="text-base text-[var(--home-text-muted)] leading-relaxed mb-8">Our mixologists are alchemists, blending local botanicals with premium spirits to create unique sensory experiences. The Nightscape Lounge is more than a bar; it's a sanctuary for the discerning.</p>
          <div className="grid grid-cols-2 gap-4">
            {['Signature Cocktails', 'Rare Spirits', 'Bespoke Service', 'Acoustic Excellence'].map((item, i) => (
              <Reveal key={item} delay={`delay-${i * 200}`}>
                <div className="p-4 rounded-2xl bg-[var(--home-card-bg)] border border-[var(--home-card-border)] text-center">
                  <span className="text-[var(--home-heading)] font-black uppercase tracking-widest text-[10px]">{item}</span>
                </div>
              </Reveal>
            ))}
          </div>
...
const GamesShowcase: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  return (
    <section id="games" className="flex flex-col justify-center px-4 md:px-12 lg:max-w-[67vw] mx-auto bg-[var(--home-bg)] transition-colors duration-500 relative items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="md:w-1/2">
          <Reveal>
            <span className="text-primary font-bold tracking-[0.3em] uppercase text-sm mb-4 block">The Arena</span>
            <h2 className="text-5xl md:text-6xl font-black text-[var(--home-heading)] mb-6 font-heading leading-[0.85] uppercase tracking-tighter">
              HIGH<br /><span className="text-[var(--home-primary)]">Performance</span>
            </h2>
            <p className="text-[var(--home-text-muted)] text-base leading-relaxed mb-8 font-medium max-w-xl">Step into the future of competitive gaming. From high-refresh rate PC arenas to immersive VR pods, we provide the infrastructure for champions.</p>
            <button onClick={() => { setCurrentView('games'); document.getElementById('main-scroll-container')?.scrollTo({ top: 0, behavior: 'smooth' }); }} className="group flex items-center gap-4 text-[var(--home-heading)] border-b-2 border-[var(--home-primary)] pb-2 hover:text-[var(--home-primary)] transition-all uppercase text-xs font-black tracking-[0.3em] font-heading">
        ...
        <div className="md:w-1/2">
          <Reveal animation="slide-from-right">
            <ParallaxImage src="https://images.unsplash.com/photo-1542751371-adc38448a05e?q=80&w=2070&auto=format&fit=crop" alt="Gaming Arena" className="rounded-3xl h-[45vh] border border-[var(--home-border)] shadow-elite" />
          </Reveal>
        </div>
      </div>
    </section>
...
const GamesDeepDive: React.FC = () => {
  return (
    <div className="flex flex-col justify-center px-6 bg-transparent border-t border-[var(--home-border)] relative items-center h-screen">
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <Reveal animation='slide-from-right'>
          <span className="text-[var(--home-primary)] font-black tracking-[0.3em] uppercase text-sm mb-4 block">Tech Ecosystem</span>
          <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter mb-6 leading-tight">ZERO LAG <br /><span className="text-[var(--home-primary)]">Infrastructure</span></h2>
          <p className="text-base text-[var(--home-text-muted)] leading-relaxed mb-8 font-medium">We've built a dedicated fiber-optic network to ensure sub-10ms latency for competitive play. Our hardware is refreshed quarterly, featuring the latest RTX GPUs and high-fidelity VR peripherals.</p>
          <div className="space-y-4">
            {['Fiber-Optic Backbone', 'RTX 40-Series GPUs', '240Hz Displays'].map(spec => (
              <div key={spec} className="flex items-center justify-between p-6 rounded-3xl bg-[var(--home-card-bg)] border border-[var(--home-card-border)] shadow-soft">
                <span className="text-[var(--home-heading)] font-bold uppercase tracking-widest text-xs">{spec}</span>
                <span className="material-icons text-[var(--home-primary)] text-xl">bolt</span>
              </div>
            ))}
          </div>
        </Reveal>
        <Reveal animation='rotate-in'>
          <div className="relative aspect-video rounded-3xl overflow-hidden border border-[var(--home-border)] shadow-elite">
...
const MagneticCard: React.FC<{
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-[3rem] overflow-hidden cursor-pointer border border-[var(--home-border)] bg-[var(--home-card-bg)] shadow-soft hover:shadow-elite transition-all duration-500 group ${className}`}
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-[var(--home-bg)] via-transparent to-transparent opacity-90" />
      ...
      <div className="absolute inset-0 p-10 flex flex-col justify-end items-start z-10">
        <span className="material-icons text-[var(--home-primary)] text-4xl mb-4 group-hover:scale-125 transition-transform duration-500">{icon}</span>
        <h3 className={`${titleSize} font-black text-[var(--home-heading)] font-heading uppercase tracking-tight leading-none group-hover:translate-x-2 transition-transform duration-500`}>{title}</h3>
        {subtitle && !className.includes('h-') && (
          <p className="text-[10px] text-[var(--home-text-muted)] mt-2 font-black tracking-[0.3em] uppercase opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">{subtitle}</p>
        )}
      </div>
    ...
    <section className="relative z-30 bg-transparent h-screen flex flex-col justify-center px-4 sm:px-8 items-center">
      <div className="lg:max-w-[67vw] mx-auto w-full">
        <Reveal>
          ...
          <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter max-w-3xl mb-8">
            FOR THOSE WHO DEMAND <span className="opacity-40 italic">EXCELLENCE.</span>
          </h2>
        </Reveal>
...
const TrustSection: React.FC = () => {
  ...
  return (
    <section className="flex flex-col justify-center px-6 relative overflow-hidden bg-transparent border-t border-[var(--home-border)] items-center h-screen">
      ...
      <div className="lg:max-w-[67vw] mx-auto flex flex-col justify-center">
        <Reveal>
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter leading-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {certifications.map((cert, i) => (
            <Reveal key={cert.name} delay={`delay-${i * 300}`}>
              <div
                className="p-8 rounded-3xl bg-[var(--home-card-bg)] border border-[var(--home-card-border)] shadow-soft group hover:border-primary/50 transition-all duration-700 relative overflow-hidden"
              >
                ...
                <h3 className="text-xl font-black text-[var(--home-heading)] uppercase mb-4 tracking-tight leading-none">{cert.name}</h3>
                <p className="text-[var(--home-text-muted)] leading-relaxed font-medium text-sm">{cert.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
const LocationSection: React.FC = () => {
  return (
    <section className="flex flex-col justify-center px-6 bg-transparent border-t border-[var(--home-border)] relative overflow-hidden items-center h-screen">
      ...
      <div className="lg:max-w-[67vw] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <Reveal animation='slide-from-left'>
          ...
          <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter mb-6 leading-tight">
            RAYFIELD, <br /><span className="text-primary">Jos</span>
          </h2>
          <p className="text-base text-[var(--home-text-muted)] leading-relaxed mb-8 font-medium">
          ...
          <div className="space-y-4">
            <Reveal delay='delay-75'>
              <div className="flex items-center gap-6 p-6 rounded-3xl bg-[var(--home-card-bg)] border border-[var(--home-card-border)] shadow-soft group hover:border-primary/30 transition-all duration-500">
                ...
                <div>
                  <h4 className="text-[var(--home-heading)] font-black uppercase tracking-tight text-lg mb-1">Amanda Plaza</h4>
                  <p className="text-[var(--home-text-muted)] text-sm font-medium">Rayfield, Jos, Plateau State</p>
                </div>
              </div>
            </Reveal>
            <Reveal delay='delay-150'>
              <div className="flex items-center gap-6 p-6 rounded-3xl bg-[var(--home-card-bg)] border border-[var(--home-card-border)] shadow-soft group hover:border-primary/30 transition-all duration-500">
                ...
                <div>
                  <h4 className="text-[var(--home-heading)] font-black uppercase tracking-tight text-lg mb-1">Open Daily</h4>
                  <p className="text-[var(--home-text-muted)] text-sm font-medium">8:00 AM - 11:00 PM</p>
                </div>
              </div>
            </Reveal>
          </div>
        ...
        <Reveal animation='slide-from-right'>
          <div className="relative aspect-square rounded-3xl overflow-hidden border border-[var(--home-border)] shadow-elite">
  ...
  const testimonials = [
    {
      ...
      quote: "Orient Global has transformed our community. The quality of their products and services is unmatched, and their commitment to local development is truly inspiring.",
    ...
    },
    {
      ...
      quote: "The standards at Orient Water are exemplary. Their dedication to purity and safety sets a new benchmark for the region. A vital asset for public health.",
      img: "https://images.unsplash.com/photo-1559839734-2b716b178d9a?q=80&w=1964&auto=format&fit=crop"
    },
    {
      name: "Grace Davou",
      title: "Culinary Critic",
      quote: "Dining at Orient Global's restaurant is an experience in itself. The fusion of local flavors with international techniques is simply brilliant. A must-visit for food lovers.",
      img: "https://images.unsplash.com/photo-1508214751196-cdfd462bc550?q=80&w=2070&auto=format&fit=crop"
    },
    {
      name: "Chidi Okoro",
      title: "Gaming Enthusiast",
      quote: "The VR Frontier at Orient Global is mind-blowing! Cutting-edge tech and an immersive environment make it the best gaming spot in Jos. Always a blast!",
      img: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=1974&auto=format&fit=crop"
    }
  ];

  return (
    <section className="bg-[var(--home-bg)] border-t border-[var(--home-border)] py-20 transition-colors duration-500 relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(242,158,13,0.02),transparent_50%)]" />
      <div className="lg:max-w-[67vw] mx-auto px-6 relative z-10">
        <Reveal>
          <span className="text-primary font-black tracking-[0.3em] uppercase text-sm mb-4 block text-center">
            Voices of Jos
          </span>
          <h2 className="text-4xl sm:text-5xl font-black text-[var(--home-heading)] uppercase tracking-tighter text-center max-w-3xl mx-auto mb-16">
            WHAT OUR COMMUNITY <span className="opacity-40 italic">SAYS.</span>
          </h2>
        </Reveal>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {testimonials.map((testimonial, i) => (
            <Reveal key={testimonial.name} delay={0.2 * i}>
              <div className="p-8 rounded-[3rem] bg-[var(--home-card-bg)] border border-[var(--home-border)] shadow-soft hover:shadow-elite transition-all duration-500 h-full flex flex-col">
                <div className="flex items-center gap-4 mb-6">
                  <img src={testimonial.img} alt={testimonial.name} className="w-14 h-14 rounded-full object-cover border-2 border-primary" />
                  <div>
                    <h4 className="text-lg font-black text-[var(--home-heading)] leading-none">{testimonial.name}</h4>
                    <p className="text-xs text-[var(--home-text-muted)] uppercase tracking-widest font-bold mt-1">{testimonial.title}</p>
                  </div>
                </div>
                <p className="text-sm text-[var(--home-text-muted)] leading-relaxed font-medium flex-grow">"{testimonial.quote}"</p>
              </div>
            </Reveal>
          ))}
        </div>
...
const Footer: React.FC<{ setCurrentView: (v: any) => void }> = ({ setCurrentView }) => {
  ...
  return (
    <footer className="bg-[var(--home-bg)] border-t border-[var(--home-border)] py-20 transition-colors duration-500 relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_right,rgba(242,158,13,0.02),transparent_50%)]" />
      <div className="lg:max-w-[67vw] mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          <div className="col-span-1 lg:col-span-1">
            <h2 className="text-2xl font-black text-[var(--home-heading)] mb-6 uppercase tracking-tighter leading-none">
              ORIENT<br /><span className="text-primary italic font-serif normal-case tracking-normal text-xl">Global</span>
            </h2>
            <p className="text-xs text-[var(--home-text-muted)] leading-relaxed font-medium mb-8 max-w-xs uppercase tracking-widest">The standard of excellence on the Plateau since 1996.</p>
            <div className="flex gap-4">
              {['facebook', 'instagram', 'twitter', 'linkedin'].map(social => (
                <a key={social} href="#" className="w-10 h-10 rounded-xl bg-[var(--home-bg-soft)] border border-[var(--home-border)] flex items-center justify-center text-[var(--home-heading)] hover:bg-primary hover:text-white transition-all duration-500 shadow-sm">
                  <span className="material-icons text-xl">{social === 'twitter' ? 'X' : social}</span>
                </a>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-[var(--home-heading)] font-black uppercase tracking-[0.2em] mb-6 text-xs">Divisions</h3>
            <ul className="space-y-4">
              {['Bakery', 'Supermarket', 'Restaurant', 'Water'].map(item => (
                <li key={item}><a className="text-xs text-[var(--home-text-muted)] hover:text-primary transition-colors font-bold uppercase tracking-widest flex items-center gap-2" href="#"><span className="w-1 h-1 bg-primary/30 rounded-full" /> {item}</a></li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-[var(--home-heading)] font-black uppercase tracking-[0.2em] mb-6 text-xs">Company</h3>
            <ul className="space-y-4">
          ...
          <div>
            <h3 className="text-[var(--home-heading)] font-black uppercase tracking-[0.2em] mb-6 text-xs">Newsletter</h3>
            <p className="text-xs text-[var(--home-text-muted)] mb-4 leading-relaxed font-medium">Subscribe to receive corporate updates and exclusive offers.</p>
            <div className="relative">
              <input className="w-full bg-[var(--home-card-bg)] border border-[var(--home-card-border)] rounded-xl py-3 px-4 text-[var(--home-heading)] placeholder-gray-500 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-sm font-medium shadow-inner" placeholder="Email Address" type="email" />
            ...
            <p className="text-[9px] text-[var(--home-text-muted)] mt-4 uppercase tracking-widest font-black">By subscribing, you agree to our Privacy Policy.</p>
          </div>
        </div>

        <div className="border-t border-[var(--home-border)] pt-8 flex flex-col md:flex-row justify-between items-center gap-6 text-[9px] text-[var(--home-text-muted)] uppercase tracking-[0.2em] font-black">
...
const ChatBot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, { role: 'bot', text: "Thank you for your message. Our AI Concierge is currently undergoing maintenance. Please explore our divisions using the navigation menu." }]);
      setIsTyping(false);
    }, 1000);
  };
...
const MainLayout: React.FC<{
  currentView: 'home' | 'login' | 'admin' | 'bakery' | 'supermarket' | 'dining' | 'games' | 'water' | 'lounge' | 'about' | 'auth-login' | 'auth-signup' | 'checkout' | 'success';
  setCurrentView: (view: any) => void;
  bakeryView: 'home' | 'menu' | 'architect' | 'wholesale' | 'story';
  setBakeryView: (view: any) => void;
  supermarketPage: SupermarketPage;
  setSupermarketPage: (page: SupermarketPage) => void;
  diningView: DiningView;
  setDiningView: (view: DiningView) => void;
  gamesPage: GamesPage;
  setGamesPage: (page: GamesPage) => void;
  waterPage: WaterPage;
  setWaterPage: (page: WaterPage) => void;
  loungePage: LoungePage;
  setLoungePage: (page: LoungePage) => void;
  globalTheme: 'dark' | 'light';
  divisionThemes: Record<string, 'dark' | 'light'>;
  ...
  setIsSmartPasteOpen: (isOpen: boolean) => void;
  currentUser: any;
  handleLogin: (user?: any) => void;
  isNavHovered: boolean;
  setIsNavHovered: (isHovered: boolean) => void;
  heroOutOfView: boolean;
  setHeroOutOfView: (outOfView: boolean) => void;
  smSearchOutOfView: boolean;
  setSmSearchOutOfView: (out: boolean) => void;
  smSearchTerm: string;
  setSmSearchTerm: (term: string) => void;
}> = ({
  currentView, setCurrentView, bakeryView, setBakeryView, supermarketPage, setSupermarketPage,
  diningView, setDiningView, gamesPage, setGamesPage, waterPage, setWaterPage,
  loungePage, setLoungePage, globalTheme, divisionThemes, toggleGlobalTheme,
  scrolled, setScrolled, navHidden, setNavHidden, activeSectionId, setActiveSectionId, isLoading,
  isSmartPasteOpen, setIsSmartPasteOpen, currentUser, handleLogin, isNavHovered, setIsNavHovered, heroOutOfView, setHeroOutOfView,
  smSearchOutOfView, setSmSearchOutOfView, smSearchTerm, setSmSearchTerm
}) => {
    ...
    useEffect(() => {
      ...
      const handleScroll = () => {
        ...
        if (!hasCheckedHero) {
          const heroSection = container.querySelector('header, .cinematic-section, [data-hero="true"]');
          if (heroSection) {
            const heroRect = heroSection.getBoundingClientRect();
            const heroHeight = heroRect.height;

            if (heroHeight < vh * 0.5) {
              setHeroOutOfView(true);
            } else {
              setHeroOutOfView(currentScrollY >= heroHeight);
            }
          } else {
            setHeroOutOfView(true);
          }
        ...
        // If hero is out of view, we are definitely "scrolled"
        setScrolled(heroOutOfView);
    ...
    }, [currentView, setScrolled, setNavHidden]); // Re-init on view change only
    ...
    return (
      <motion.div
        ...
        className="theme-transition bg-slate-50 dark:bg-background-dark text-slate-900 dark:text-white selection:bg-primary selection:text-black relative z-10 h-[100dvh] overflow-hidden"
      >
        ...
        <div ref={scrollContainerRef} className="w-full mx-auto h-[100dvh] overflow-y-auto overflow-x-hidden scroll-smooth bg-transparent [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']" id="main-scroll-container">
          <ScrollContext.Provider value={{ scrollContainerRef, activeSectionId, setActiveSectionId }}>
            {currentView === 'home' && (
              <div className="relative">
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
            ...
            {(currentView === 'login' || currentView === 'admin') && (
              <div className="min-h-screen">
                <AdminDashboard onCancel={() => setCurrentView('home')} />
              </div>
            )}

            {currentView === 'bakery' && (
              <div className={`bakery-theme ${divisionThemes['bakery'] === 'dark' ? 'dark' : ''} min-h-screen relative`}>
                ...
                <BakeryNav navHidden={navHidden} currentView={bakeryView} setView={setBakeryView} setGlobalView={setCurrentView} currentUser={currentUser} scrolled={scrolled} isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} heroOutOfView={heroOutOfView} />
                <BakeryApp currentView={bakeryView} setCurrentView={setCurrentView} currentUser={currentUser} />
                <Footer setCurrentView={setCurrentView} />
              </div>
            )}

            {currentView === 'supermarket' && (
              <div className={`supermarket-theme ${divisionThemes['supermarket'] === 'dark' ? 'dark' : ''} min-h-screen relative`}>
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />              <SupermarketNav
                  navHidden={navHidden}
                  activePage={supermarketPage}
                  setActivePage={setSupermarketPage}
                  setIsSmartPasteOpen={setIsSmartPasteOpen}
                  scrolled={scrolled}
                  isAppNavHovered={isNavHovered}
                  onHoverChange={setIsNavHovered}
                  heroOutOfView={heroOutOfView}
                  searchOutOfView={smSearchOutOfView}
                  searchTerm={smSearchTerm}
                  onSearch={setSmSearchTerm}
                />
                <SupermarketApp
                  activePage={supermarketPage}
                  setActivePage={setSupermarketPage}
                  ...
                  setIsSmartPasteOpen={setIsSmartPasteOpen}
                  searchOutOfView={smSearchOutOfView}
                  setSearchOutOfView={setSmSearchOutOfView}
                  searchTerm={smSearchTerm}
                  setSearchTerm={setSmSearchTerm}
                />
            ...
            {currentView === 'dining' && (
              <div className={`dining-theme ${divisionThemes['dining'] === 'dark' ? 'dark' : ''} min-h-screen relative`}>
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
                <DiningNav navHidden={navHidden} currentView={diningView} setView={setDiningView} scrolled={scrolled} isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} heroOutOfView={heroOutOfView} />
            ...
            {currentView === 'games' && (
              <div className={`games-theme ${divisionThemes['games'] === 'dark' ? 'dark' : ''} min-h-screen relative`}>
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
                <GamesNav navHidden={navHidden} currentPage={gamesPage} onNavigate={setGamesPage} scrolled={scrolled} isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} heroOutOfView={heroOutOfView} />
            ...
            {currentView === 'lounge' && (
              <div className={`lounge-theme ${divisionThemes['lounge'] === 'dark' ? 'dark' : ''} min-h-screen relative`}>
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
                <LoungeNav navHidden={navHidden} currentPage={loungePage} onNavigate={setLoungePage} scrolled={scrolled} isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} heroOutOfView={heroOutOfView} />
            ...
            {currentView === 'auth-login' && (
              <div className="relative min-h-screen bakery-theme bg-[var(--bakery-bg)] flex items-center justify-center pt-24">
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
                <LoginForm onLogin={handleLogin} onSwitchToSignup={() => setCurrentView('auth-signup')} />
              </div>
            )}
            ...
            {currentView === 'auth-signup' && (
              <div className="relative min-h-screen bakery-theme bg-[var(--bakery-bg)] flex items-center justify-center pt-24">
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
                <SignupForm onSignup={handleLogin} onSwitchToLogin={() => setCurrentView('auth-login')} />
              </div>
            )}

            {currentView === 'checkout' && (
              <div className="relative min-h-screen bakery-theme bg-[var(--bakery-bg)] pt-32 pb-20">
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
                <CheckoutPage onComplete={() => setCurrentView('success')} />
              </div>
            )}

            {currentView === 'success' && (
              <div className="relative min-h-screen bakery-theme bg-[var(--bakery-bg)] flex items-center justify-center flex-col text-center px-4">
                <Navbar theme={globalTheme} toggleTheme={toggleGlobalTheme} setCurrentView={setCurrentView} scrolled={scrolled} navHidden={navHidden} isSubpage isAppNavHovered={isNavHovered} onHoverChange={setIsNavHovered} />
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="p-12 bg-[var(--bakery-card-bg)] rounded-[3rem] border border-[var(--bakery-card-border)] shadow-2xl">
                  <div className="w-20 h-20 bg-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 text-white">
                    <span className="material-icons text-4xl">check_circle</span>
                  </div>
                  <h2 className="text-3xl font-black text-[var(--bakery-heading)] mb-4">Order Successful!</h2>
                  <p className="text-[var(--bakery-text-muted)] mb-8">Your fresh batch of Orient goodies is being prepared.</p>
                  <button onClick={() => setCurrentView('bakery')} className="bg-primary text-white font-bold py-4 px-10 rounded-2xl shadow-lg hover:shadow-primary/30 transition-all">
                    Return to Bakery
                  </button>
                </motion.div>
              </div>
            ...
            {currentView !== 'login' && (
              <DynamicUtilityDock
                ...
                ctaHref="/#booking"
                context={currentView === 'bakery' ? 'bakery' : currentView === 'water' ? 'water' : currentView === 'supermarket' ? 'market' : currentView === 'lounge' ? 'lounge' : 'default'}
              />
            )}
...
const App: React.FC = () => {
  ...
  const [divisionThemes, setDivisionThemes] = useState<Record<string, 'dark' | 'light'>>({
    home: 'light', login: 'light', admin: 'light', bakery: 'light',
    supermarket: 'light', dining: 'light', games: 'light', water: 'light', lounge: 'light'
  });
  const [isLoading, setIsLoading] = useState(true);
  const [currentView, setCurrentView] = useState<'home' | 'login' | 'admin' | 'bakery' | 'supermarket' | 'dining' | 'games' | 'water' | 'lounge' | 'about' | 'auth-login' | 'auth-signup' | 'checkout' | 'success'>('home');
  ...
  const [smSearchOutOfView, setSmSearchOutOfView] = useState(false);
  ...
  useEffect(() => {
    ...
    setHeroOutOfView(false); // Reset on view change
  }, [currentView]);

  useEffect(() => {
    const activeTheme = divisionThemes[currentView] || 'light';
  ...
  // Merged authentication logic
  const handleLogin = (user?: any) => {
    if (user) {
      setCurrentUser(user);
      // If the admin logs in via the site portal, send them to the executive dashboard
      if (user.email === 'ceo@og.com') {
