        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {certifications.map((cert, i) => (
            <Reveal key={cert.name} delay={`delay-${i * 300}`}>
              <div
                className="p-8 rounded-3xl bg-white dark:bg-zinc-900/40 border border-black/5 dark:border-white/5 shadow-soft group hover:border-primary/50 transition-all duration-700 relative overflow-hidden"
              >
...
const MainLayout: React.FC<{
  ...
  setCurrentView: (v: any) => void;
  bakeryView: any;
  setBakeryView: (v: any) => void;
  supermarketPage: any;
  setSupermarketPage: (v: any) => void;
  diningView: any;
  setDiningView: (v: any) => void;
  gamesPage: any;
  setGamesPage: (v: any) => void;
  waterPage: any;
  setWaterPage: (v: any) => void;
  loungePage: any;
  setLoungePage: (v: any) => void;
  globalTheme: 'dark' | 'light';
  divisionThemes: Record<string, 'dark' | 'light'>;
  toggleGlobalTheme: () => void;
  toggleLocalTheme: (division: string) => void;
  scrolled: boolean;
  ...
  loungePage, setLoungePage, globalTheme, divisionThemes, toggleGlobalTheme, toggleLocalTheme,
  scrolled, setScrolled, navHidden, setNavHidden, activeSectionId, setActiveSectionId, isLoading,
  isSmartPasteOpen, setIsSmartPasteOpen
}) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-slate-50 dark:bg-background-dark transition-colors duration-700 dark:text-white text-slate-900 selection:bg-primary selection:text-black relative z-10 h-[100dvh] overflow-hidden"
    >
    ...
    <AnimatePresence mode="wait">
      {isLoading ? (
        ...
        <MainLayout 
          ...
          toggleLocalTheme={toggleLocalTheme}
          scrolled={scrolled}
